...
Predbat expects its inverter mapping in `apps.yaml`, and it ships templates for GivTCP and the GivEnergy cloud but not yet for this integration. I don't run Predbat myself, so I can't author or validate that mapping — but a community member kindly shared a working two-inverter EMS `apps.yaml` on [#95](https://github.com/dewet22/givenergy-hass/issues/95#issuecomment-4913780820), which is a solid starting point (mind that some entity IDs may differ with install vintage). If you get a config working on your own setup and are happy to share it, I'll gladly host it here so it's discoverable rather than buried in an issue thread — see [#281](https://github.com/dewet22/givenergy-hass/issues/281).
