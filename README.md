...
A **Generation** tab rounds out the deep views with a year of PV history: a density heatmap (hour-of-day against power, colour showing how often the system generated at that level — the system's envelope), a calendar heatmap (hour against date — the seasonal bulge), and weekly kWh totals. Both heatmaps keep their hour axis in fixed UTC+1 year-round so solar noon doesn't jump at clock changes; an optional `max_pv_kw:` strategy option caps the density plot's y-axis (it defaults to the observed maximum).

The three extra options name entities from other integrations — tariff rates (the Octopus Energy integration's current-rate entities) and an hourly solar forecast (Solcast's today entity). All three are optional: each missing feed drops its tape layer with a small legend note rather than breaking anything. Two conveniences are derived automatically and verified against the registry before use: the Octopus day-rates `event.*_{previous,current,next}_day_rates` siblings of each rate sensor (recent Octopus versions carry the forward rates there rather than on the sensor itself — override with `tariff_import_rates:`/`tariff_export_rates:` lists if yours differ), and the `_tomorrow` sibling of a Solcast `_today` forecast entity so the tape's forward half survives crossing midnight (`solar_forecast:` also accepts an explicit list). The Ledger tab additionally needs the **money sensors**, enabled by naming the same tariff entities in the integration's options (Settings → Devices & services → GivEnergy Local → Configure); without them the tab simply isn't emitted. The tariff-band classification (cheap / standard / peak relative to the day's median) and the SOC projection are heuristics — useful for orientation, not billing-grade. This mode is newer than the others and has had less real-world soak time.
