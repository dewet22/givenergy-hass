...
DASHBOARD_VERSION = 11

# Every givenergy_local entity reference the generator can emit. The generator
# builds *canonical* ids (`{domain}.givenergy_{kind}_{serial}_{slug}`); a caller
# can pass `resolve_entity_id` to rewrite each match to the actual registry id —
# necessary since HA 2026.6 folds the device's area into generated entity_ids
# (`sensor.loft_givenergy_inverter_…`) and users may rename entities.
...
def generate_dashboard(
    ...
    *,
    ...
    resolve_entity_id: Callable[[str], str] | None = None,
) -> str:
    ...
    Args:
        ...
        resolve_entity_id:     Optional mapping from a canonical entity id (the
                               `{domain}.givenergy_{kind}_{serial}_{slug}` form this
                               generator builds) to the actual id registered in HA.
                               Required for correctness since HA 2026.6 prefixes
                               generated entity_ids with the device's area; without it
                               the canonical (area-less) ids are emitted unchanged.
    """
    if is_ems:
    ...
    if is_ems:
        result = (
            ...
            f"# Entity ids reflect the registry at generation time; re-run this service\n"
            f"# after renaming entities or moving devices between areas.\n"
            f"\n"
    ...
    # HA 2026.6 prefixes generated entity_ids with the device's area, and users may
    # rename entities — so the canonical ids built above can be wrong. Rewrite each to
    # the actual registry id when a resolver is supplied (unresolved refs pass through).
    if resolve_entity_id is not None:
        result = _ENTITY_REF.sub(lambda m: resolve_entity_id(m.group(0)), result)
    return result
